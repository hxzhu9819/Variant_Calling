...
public final class PairHMMLikelihoodCalculationEngine implements ReadLikelihoodCalculationEngine {
    ...
    // added by Chenhao: get the pairHMM tool
    ...
    //Prune Method
    @Override
    public List<ReadLikelihoods<Haplotype>> computeReadLikelihoods( final AssemblyResultSet assemblyResultSet, final SampleList samples, final Map<String, List<GATKRead>> perSampleReadList ) {
        Utils.nonNull(assemblyResultSet, "assemblyResultSet is null");
        Utils.nonNull(samples, "samples is null");
        Utils.nonNull(perSampleReadList, "perSampleReadList is null");
        ...
        for (int i = 0; i < sampleCount; i++) {
            ...
            computeReadLikelihoods(result_lowerbound.sampleMatrix(i), result_upperbound.sampleMatrix(i), result_lowerbound.gapPenalties.get(i), result_lowerbound.processedReadsList.get(i));
        }
        ...
        //System.out.println("cap_dif: " + log10globalReadMismappingRate);
        ...
        //int job_tot = 0;
        //for(int i=0;i<sampleCount;i++){
        //    for(int g=0; g<result_lowerbound.sampleMatrix(i).alleles().size();g++){
        //        for(int r=0;r<result_lowerbound.sampleMatrix(i).reads().size();r++){
    ...
    private void computeReadLikelihoods(final LikelihoodMatrix<Haplotype> likelihoods_lowerbound,final LikelihoodMatrix<Haplotype> likelihoods_upperbound, Map<GATKRead, byte[]> gapContinuationPenalties, List<GATKRead> processedReads) {
        ...
        // Run the PairHMM to calculate the log10 likelihood of each (processed) reads' arising from each haplotype
        pairHMM.computeLog10Likelihoods(likelihoods_lowerbound,likelihoods_upperbound, processedReads, gapContinuationPenalties);

        //pairHMM.close();//write out profiling time
        writeDebugLikelihoods(likelihoods_lowerbound);
    }

    // added by Chenhao: generate the GapContinuationPenalties
    ...
    // added by Chenhao: generate the modified reads
